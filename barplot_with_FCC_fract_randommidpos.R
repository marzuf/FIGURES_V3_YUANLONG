...
# Rscript barplot_with_FCC_fract_randommidpos.R
...
outFolder <- "BARPLOT_WITH_FCC_FRACT_RANDOMMIDPOS"
...
auc_ratio_file <- file.path("BARPLOT_FCC_AUC_RATIO_RANDOMMIDPOS", "all_dt.Rdata")
...
rd_dt <- get(load("BARPLOT_FCC_AUC_RATIO_RANDOMMIDPOS//all_dt.Rdata"))
rd_dt$hicds <- gsub("_RANDOMMIDPOS_40kb", "_40kb", rd_dt$hicds)
...
outFile <- file.path(outFolder, paste0("all_ds_fcc_auc_randommidpos_vs_obs.", plotType))
...
plot(
  ...
  ylab="FCC AUC (RANDOMMIDPOS)",
  main = paste0("OBS vs. RANDOMMIDPOS - FCC AUC"),
  cex.main=plotCex,
  cex.axis=plotCex,
  cex.lab=plotCex,
  pch = 16
...
outFile <- file.path(outFolder, paste0("all_ds_coexpr_auc_randommidpos_vs_obs.", plotType))
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
par(bty="L")
plot(
  ...
  ylab="COEXPR AUC (RANDOMMIDPOS)",
  main = paste0("OBS vs. RANDOMMIDPOS - COEXPR AUC"),
  cex.main=plotCex,
  cex.axis=plotCex,
  cex.lab=plotCex,
  pch = 16
...
outFile <- file.path(outFolder, paste0("all_ds_coexpr_vs_fcc_auc_randommidpos.", plotType))
do.call(plotType, list(outFile, height=myHeight, width=myWidth))
par(bty="L")
plot(
  x=merged_dt$fcc_auc_rd,
  y=merged_dt$coexpr_auc_rd,
  xlab="FCC AUC (RANDOMMIDPOS)",
  ylab="COEXPR AUC (RANDOMMIDPOS)",
  main = paste0("COEXPR vs. FCC AUC - RANDOMMIDPOS"),
  cex.main=plotCex,
  cex.axis=plotCex,
  cex.lab=plotCex,
  pch = 16
